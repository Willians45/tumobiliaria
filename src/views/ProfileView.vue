  ...
  <div class="pt-20 pb-24 px-4 min-h-screen bg-gray-50">
    ...
    <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mb-6 text-center">
      <div class="w-20 h-20 bg-rose-100 rounded-full flex items-center justify-center mx-auto mb-4 overflow-hidden">
        ...
        <User v-else :size="40" class="text-rose-500" />
      </div>
      
      <div v-if="isGuest">
        <h2 class="text-xl font-bold text-gray-900">Invitado</h2>
        <p class="text-gray-500 text-sm mb-4">Inicia sesión para puntuar y gestionar propiedades</p>
      </div>
      <div v-else>
        <h2 class="text-xl font-bold text-gray-900">{{ currentUser.name }}</h2>
        <p class="text-sm text-green-600 font-medium flex items-center justify-center gap-1 mb-4">
            <CheckCircle2 :size="14" /> Sesión Iniciada
        </p>
      </div>
    ...
    <div v-if="isGuest" class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
      ...
      <div class="relative my-6">
        ...
        <div class="relative flex justify-center text-sm">
          <span class="px-2 bg-white text-gray-500 text-xs">O continúa con</span>
        </div>
      </div>

      <button @click="handleAuthAction" class="w-full bg-white border border-gray-200 text-gray-700 font-bold py-2.5 rounded-xl hover:bg-gray-50 transition-colors flex items-center justify-center gap-2">
